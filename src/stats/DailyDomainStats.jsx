 ...
 */
function getSimplifiedDomainName(domain) {
    // The content script already handles removing "www.",
    // so we can directly split the domain to get the core name.
...
export function DailyDomainStats() {
    ...
    if (isLoading) {
        return <div className="text-gray-400 text-center">Loading daily statistics...</div>;
    }
    ...
    if (domainStats.length === 0) {
        return <p className="text-gray-500 text-center">No usage data for today.</p>;
    }

    return (
        <>
            <h2 className="text-2xl font-bold text-left text-gray-700 mb-6 mt-6 ml-2.5">Daily Domain Usage</h2>
